...
import pdb
import json
from my_prompt import sft_prompt, all_prompt
import numpy as np
...
class BaseDataset(Dataset):
    ...
    def _load_data(self):
        ...
        with open(os.path.join(self.data_path, self.user_index_file), 'r') as f:
...
class SeqRecDataset(BaseDataset):
    ...
    def _process_train_data(self):
        ...
        for uid  in self.remapped_inters:
            user_id = "".join(self.user_indices[uid])
...
class UserFeatDataset(BaseDataset):
    ...
    def __init__(self, args, task="user2index", prompt_sample_num=1, sample_num=-1):
        super().__init__(args)
    ...
    def _load_data(self):
        ...
        with open(os.path.join(self.data_path, self.dataset + ".user.json"), 'r') as f:
            self.user_feat = json.load(f)


    def _process_data(self):

        feat_data = []
        for iid in self.user_feat:
            feat = self.user_feat[iid]
            index = "".join(self.indices[iid])
            feat["user"] = index
            feat["title"] = feat["profile"].strip().strip(".!?,;:`")
            feat_data.append(feat)
