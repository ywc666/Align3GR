...
from mydata import SeqRecDataset, ItemFeatDataset, ItemSearchDataset, FusionSeqRecDataset, SeqRecTestDataset, PreferenceObtainDataset, UserFeatDataset
...
def parse_global_args(parser):
    ...
    parser.add_argument("--base_model", type=str,default="LLM-Research/llama-2-7b", help="basic model path")
    parser.add_argument("--output_dir", type=str,default="./ckpt/Instruments/", help="The output directory")

    return parser

def parse_dataset_args(parser):
    parser.add_argument("--data_path", type=str, default="Align3GR/data/",help="data directory")
    parser.add_argument("--tasks", type=str, default="seqrec", 
                        help="Downstream tasks, separate by comma") # seqrec,item2index,index2item,user2index,index2user,fusionseqrec,itemsearch,preferenceobtain
    parser.add_argument("--dataset", type=str, default="Instruments", help="Dataset name")
    parser.add_argument("--index_file", type=str, default="Instruments_item.index.json", help="the item indices file")
    parser.add_argument("--user_index_file", type=str, default="Instruments_user.index.json", help="the user indices file")
    ...
    parser.add_argument("--train_data_sample_num", type=str, default="0", # 0 means all data
                        help="the number of sampling prompts for each task")
...
def parse_train_args(parser):
    ...
    parser.add_argument("--resume_from_checkpoint", type=str, default=None, help="either training checkpoint or final adapter") # 是否从checkpoint恢复训练
...
def parse_test_args(parser):
    ...
    parser.add_argument("--ckpt_path", type=str,default="Align3GR/SFT/ckpt/Instruments/v2/checkpoint-256",help="The checkpoint path")
    ...
    parser.add_argument("--results_file", type=str,default="./results/Instruments/test.json",help="result output path")
...
def load_datasets(args):
    ...
    for task, prompt_sample_num, data_sample_num in zip(tasks,train_prompt_sample_num,train_data_sample_num):
        ...
        elif task.lower() == "user2index" or task.lower() == "index2user":
