...
class DINTrain:
    def __init__(self,item_num=100,sample_negative_num=60,emb_dim=96,device='cpu',
        ...
        self.DINModel=DeepInterestNetwork(item_num=item_num,embedding_dim=emb_dim, feature_groups=feature_groups,sum_pooling=sum_pooling).to(self.device)
    ...
    def uniform_sampled_softmax(self,batch_users,batch_labels,N):
        ...
        log_q_matrix[:,1:][effective_index[:,1:]]= torch.log(effective_index[:,1:].sum(-1).view(batch_size,1)*1.0/(self.item_num-1)).expand(batch_size,N)[effective_index[:,1:]]
        ...
        o_pi[effective_index] = self.DINModel(batch_users[user_index],samples[effective_index].view(-1, 1))[:, 0] - log_q_matrix[effective_index]
