...
class DeepInterestNetwork(nn.Module):
    def __init__(self, item_num=100,embedding_dim=96,
        ...
        self.item_embedding=EmbeddingLayer(item_num,96)
            # nn.Linear(768, 256),
            # nn.ReLU(inplace=True),
            # nn.Linear(256, embedding_dim))

        self.attention_unit=LocalActivationUnit(hidden_size=[64, 16], bias=[True, True], embedding_dim=embedding_dim, batch_norm=False)
        if sum_pooling:    
            ...
            self.fc_layer = FullyConnectedLayer(input_size=(len(feature_groups)+1) *embedding_dim, hidden_size=[200, 80, 1], bias=[True, True, True],
                                                activation='dice',
                                                sigmoid=False)
    ...
    def forward(self, batch_user,batch_label):
        ...
        if self.sum_pooling:
            ...
            weight_matrix[effective_index]=self.attention_unit(self.item_embedding(batch_user[effective_index]), self.item_embedding(effective_labels)).view(-1)
            return self.fc_layer(
                torch.cat((torch.matmul(weight_matrix.view(batch_size,1,f_num),self.item_embedding(batch_user)), self.item_embedding(batch_label.view(-1))),dim=1))
        else:
            pre_linear_part_inputs = torch.zeros((batch_size * f_num, self.embed_dim),device=batch_user.device,dtype=torch.float32) # 创建预线性层输入矩阵 

            pre_linear_part_inputs[effective_index.view(-1)]= self.item_embedding(batch_user[effective_index])* self.attention_unit(self.item_embedding(batch_user[effective_index]),self.item_embedding(effective_labels))
            a = self.window_matrix.size()
            b = pre_linear_part_inputs.view(batch_size,f_num,self.embed_dim).size()


            return self.fc_layer(
                torch.cat((torch.matmul(self.window_matrix,pre_linear_part_inputs.view(batch_size,f_num,self.embed_dim)).view(batch_size,-1),
