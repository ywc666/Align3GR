...
class Train_instance:
    ...
    '''
    def read_all_instances_files(self,traing_instaces_path,seg_couts,item_num, his_maxtix_path=None,labels_path=None):
        if False and his_maxtix_path and labels_path:
            his_maxtix_path = torch.load(his_maxtix_path)
            labels_path = torch.load(labels_path)
            assert len(his_maxtix_path) == len(labels_path)
            return his_maxtix_path, labels_path
        his_maxtix=None
        labels=None
        for i in range(seg_couts):
            print('reading {}-th part'.format(i))
            part_his,part_labels=self.read_one_instrance_file(traing_instaces_path,i,item_num) # len(history) = 19
    ...
    def get_training_data(self,traing_instaces_path,seg_couts,item_num, his_maxtix_path=None, labels_path=None):
        history_matrix,positive_labels=self.read_all_instances_files(traing_instaces_path,seg_couts,item_num,his_maxtix_path, labels_path)
        '''
